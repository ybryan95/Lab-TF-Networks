...
netLasso[netObject_, predictorFolds_, geneFolds_, lambdas_]:= (
       ...
       AppendTo[netObject["edgeScores"],
       	Normal[Fit[{
       		Join[predictorFolds[[1,1]],predictorFolds[[1,2]]], Join[geneFolds[[1,1]],geneFolds[[1,2]]]
       		}, 
       		"BestFitParameters", FitRegularization -> {"L1", netObject["lambdasChosen"][[-1]]}
       		]]
       ];
...
Module[ {em, i, j},
	(*em is error matrix*)
	em = ConstantArray[0,{Length[lambdas],Length[geneFolds]}];
	
	Do[
		Do[
			em[[i,j]] = sumSquaredTestError[predictorFolds[[j]], geneFolds[[j]], lambdas[[i]]],
			{i,Length[lambdas]}
		],
		{j,Length[geneFolds]}
	];
...
chooseLambda[errorsMatrixLambdasByFolds_, lambdas_]:=
	Module[{me, l, i},
		...
		(*set mean of the erros*)
		me =  ConstantArray[0,Length[errorsMatrixLambdasByFolds]];
		
		Do[
			me[[i]] =  Mean[errorsMatrixLambdasByFolds[[i]]],
				{i,Length[errorsMatrixLambdasByFolds]}
		];
		
		(*choose lambda value*)
		l = lambdas[[Ordering[me,1][[1]]]];
...
scoreNet[netObject_, bindingMatrixPath_] :=
   Module[{bindingImport = sortCSVRowColumn[Import[bindingMatrixPath]], 
   	       bindingMatrix, targetGeneNames, TFNames, result},
      bindingMatrix = bindingImport[[2 ;;, 2 ;;]];
     ...
     result = evalEdges[netObject["rankedEdges"], bindingMatrix];
...
evalEdges[rankedEdges_, bindingMatrix_]:=
	Module[{z = 0, triplet, i, acc, prev, nEdge = Length[rankedEdges],nRel},
		
		triplet =  ConstantArray[{0,0,0},nEdge];
		nRel = Total[bindingMatrix,Infinity];
		
		Do[
			If[ bindingMatrix[[rankedEdges[[i,2]], rankedEdges[[i,1]]]]==1, z++
			];
			acc = N[z/i];
			prev = N[z/nRel,MachinePrecision]; 	
	
			triplet[[i]] = {N[i,MachinePrecision],acc,prev}
